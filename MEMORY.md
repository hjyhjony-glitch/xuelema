...
## 3. 团队协同工作流程（2026-02-19）

### 3.1 团队职责分工

| 角色 | 名称 | 核心职责 |
|------|------|----------|
| **RUNBOT** | 主协调 | 任务协调、唤醒团队、自检清单、对外沟通 |
| **RUNBOT-ARC** | 青岩 | 高级分析、代码审核、质量把关、问题根因分析 |
| **RUNBOT-DEV** | 笑天 | 技术实施、代码开发、Git操作、系统构建 |
| **RUNBOT-ASS** | 书瑶 | 文档管理、全程记录、进度跟踪、决策归档 |

### 3.2 问题修复标准流程

```
发现问题 → 分配给 ARC 分析 → ARC 制定修复方案 → DEV 执行修复 → ARC 验收 → 推送
```

| 步骤 | 执行者 | 输入 | 输出 | 说明 |
|------|--------|------|------|------|
| **1. 问题发现** | 任何人 | 问题描述 | 问题工单 | 发现代码或构建问题 |
| **2. 问题分析** | ARC | 问题工单 | 分析报告 | 根因分析、影响范围评估 |
| **3. 方案制定** | ARC | 分析报告 | 修复方案 | 技术方案、风险评估 |
| **4. 执行修复** | DEV | 修复方案 | 代码改动 | 根据方案执行修改 |
| **5. 验收确认** | ARC | 代码改动 | 验收报告 | 代码审查、测试验证 |
| **6. 推送代码** | DEV | 验收报告 | 合并请求 | PR 或直接推送 main |

### 3.3 紧急情况处理

| 场景 | 处理方式 | 后续要求 |
|------|----------|----------|
| **阻断性问题**（构建失败） | RUNBOT 先行修复 | 24小时内 ARC 补审 |
| **简单配置修改** | RUNBOT 直接修改 | 记录到 MEMORY.md |
| **重要配置修改**（workflow、pubspec） | 必须经过 ARC 审核 | 无例外 |
| **日常功能开发** | DEV 执行，ARC 审核 | PR 或直接推送 |

### 3.4 职责边界

| 操作 | RUNBOT | ARC | DEV | ASS |
|------|--------|-----|-----|-----|
| 唤醒团队成员 | ✅ | ❌ | ❌ | ❌ |
| 代码修改 | ❌ | ❌ | ✅ | ❌ |
| 代码审核 | ❌ | ✅ | ❌ | ❌ |
